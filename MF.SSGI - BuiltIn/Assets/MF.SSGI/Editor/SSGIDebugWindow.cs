...
namespace MF.SSGI {
    public class SSGIDebugWindow : EditorWindow {
        ...
        private void DrawDebugRT() {
            ...
            if(selectedCam.name != "SceneCamera" && !selectedCam.GetComponent<SSGIController>()) {
                GUILayout.Label("Make sure the selected camera has an 'SSGIController' component attached");
                return;
            }
