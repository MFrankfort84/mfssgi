...
namespace MF.SSGI {
    ...
    public class PointLabelHandlersEditor : Editor {
        ...
        private static void OnDuringSceneGui(SceneView sceneView) {
            ...
            if (!SSGIController.ShowToggleIconInSceneView) {
                SSGIController.SSGIActive = true;
                return;
            }
            if (!SSGIController.ShowInSceneView) {
                return;
            }
            ...
            //Get correct icon
            Texture2D icon = SSGIController.SSGIActive ? icon_on : icon_off;
