...
//_CameraNormalsTexture (URP) replaced by _CameraDepthNormalsTexture (BIRP forward).
//Sampling lives in CaptureNormals.shader; SSGI.hlsl no longer needs the URP global directly.
...
//Built-in Render Pipeline GBuffer names (only used when _USE_DEFERRED is enabled).
//URP equivalents would be _GBuffer0/_GBuffer1. Not currently wired up — Phase 7 work.
extern sampler2D _CameraGBufferTexture0;
extern float4 _CameraGBufferTexture0_ST;

extern sampler2D _CameraGBufferTexture1;
extern float4 _CameraGBufferTexture1_ST;
...
float3 GetAlbedo(float2 uv, float3 screenNormal, float3 screenColor, float aaMask, out float occlusion) {
    ...
    #ifdef _USE_DEFERRED
        //BIRP deferred GBuffer layout: GBuffer0 = diffuse RGB + occlusion A, GBuffer1 = specular RGB + smoothness A.
        //Phase 7: this code path may need tuning — semantics differ slightly from URP's _GBuffer0/1.
        albedo = tex2D(_CameraGBufferTexture0, uv).rgb;

        float lum = albedo.r + albedo.g + albedo.b;
        float4 g1 = tex2D(_CameraGBufferTexture1, uv);
        occlusion = tex2D(_CameraGBufferTexture0, uv).a; //BIRP: occlusion in GBuffer0.a
